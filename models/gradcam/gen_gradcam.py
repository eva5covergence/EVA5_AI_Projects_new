        ...
        for data, target in test_loader:
            ...
            for label, predict in zip(labels, pred_num):
                if label != predict[0]:
                    ...
                    p.imshow(np.transpose(result,((1, 2, 0))))
...
def upgraded_missclassified_gradcam_grid(configs, classes, test_loader, device, model, matched=True,num_of_images = 25):
    num_images = 0
    for config in configs:
        config['arch'].to(device).eval()
    cams = [[cls.from_config(**config) for cls in (GradCAM,)] for config in configs]
    ...
    for gradcam, in cams:
        for data, target in test_loader:
            ...
            for label, predict in zip(labels, pred_num):
                if label != predict[0]:
                    num_images += 1
                    input_image = data[index]
                    unnorm_image = UnNormalize(*basic_config.data['normalize_paras'])
                    unnorm_image_result = unnorm_image(input_image)
                    mask, _ = gradcam(unnorm_image_result[np.newaxis, :])
                    heatmap, result = visualize_cam(mask, unnorm_image_result)
                    #unnorm_image = UnNormalize(*basic_config.data['normalize_paras'])
                    #unnorm_image_result = unnorm_image(result)
                    p = plt.subplot((num_of_images/5),5,num_images)
                    p.imshow(np.transpose(result.cpu(),((1, 2, 0))))
                    #p.imshow(np.transpose(result,((1, 2, 0))))
                    p.set_xticks(()); p.set_yticks(()) # remove ticks
                    p.set_title(f"Prediction: {classes[predict[0]]},\n Actual: {classes[label]}")
                index +=1
