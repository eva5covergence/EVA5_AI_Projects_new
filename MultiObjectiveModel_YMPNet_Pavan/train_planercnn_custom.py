...
from datasets_custom.plane_stero_custom_dataset import *
...
def train(options):
    ...
    # print(yolo_props[0])
    model = YMPNet(yolo_props[0], config).cuda()
    
    refine_model = RefineModel(options)
    model.cuda()
    model.train()  
    print_summary(model,'YMP Sample Model Summary',input_size=(3,448, 448))
    refine_model.cuda()
    refine_model.train()    
