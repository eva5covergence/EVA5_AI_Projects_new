...
#     def __call__(self, prediction, target, mask, device):
#         # transform predicted disparity to aligned depth
#         # print(f"DBG target dtype {target.dtype}")
#         target = target.to(dtype=torch.float32, device=device)
#         target_disparity = torch.zeros_like(target)
#         # print(f"DBG target dtype {target_disparity.dtype}")
#         # print(f"DBG mask dtype {mask.dtype}")
...
#         prediciton_depth = prediciton_depth.squeeze()
...
#         p = torch.sum(err, (0, 1)) / torch.sum(mask, (0, 1))

#         return 100 * torch.mean(p)

# def compute_depth_loss(depth_preds, targets, batch_size, device):
#     loss_sum=0
#     loss_metric = BadPixelMetric()
    
#     # resize prediction to match target
#     print(f"DBG compute_depth_loss depth_preds.shape - {depth_preds.shape}")
#     print(f"DBG compute_depth_loss targets.shape - {targets.shape}")
#     print(f"DBG compute_depth_loss batch_size - {batch_size}")
#     for pred, gt in zip(depth_preds, targets):
#         # mask; cf. project_depth_map.m in toolbox_nyu_depth_v2 (max depth = 10.0)
#         mask = (pred > 0) & (pred < 10)
#         print(f"DBG compute_depth_loss mask.shape - {mask.shape}")
#         print(f"DBG compute_depth_loss gt.shape - {gt.shape}")
#         print(f"DBG compute_depth_loss pred.shape - {pred.shape}")
...
#         loss = loss_metric(prediction, gt, mask, device)
#         loss_sum += loss
#     mean_loss = loss_sum / batch_size
#     print(f"depth image bad pixel loss_sum: {loss_sum / batch_size:.2f}")
#     return mean_loss


### SSIM value calculation between two candidate images (inference and ground truth)
...
class RMSELoss(nn.Module):
    ...
    def forward(self, prediction, target, mask):
        return sqrt(mse_loss(prediction, target, mask, reduction=self.__reduction))
...
class GradientLoss(nn.Module):
    ...
    def forward(self, prediction, target, mask):
        ...
        # print(f"DBG gradient loss - {total}")
        return total
...
class ScaleAndShiftInvariantLoss(nn.Module):
    def __init__(self, alpha=0.5, scales=4, reduction='batch-based'):
        ...
        self.__data_loss = RMSELoss(reduction=reduction)
...
def compute_depth_loss(depth_preds, targets, batch_size, device):
    rmse_gradient_loss_sum=0
    ssim_loss_sum=0
    for prediction, target in zip(depth_preds, targets):
        ...
        mask = (prediction > 0) & (prediction < 10)
        ...
        rmse_gradient_loss = ss_loss(prediction, target, mask)
        rmse_gradient_loss_sum += rmse_gradient_loss
        # print(f"depth image bad pixel rmse_gradient_loss: {rmse_gradient_loss:.2f}")
        
        ## SSIM loss calculation
        prediction = prediction.unsqueeze(0)
        target = target.unsqueeze(0)
        ssim_value = SSIM(window_size = 11)
        ssim_loss = 1-ssim_value(prediction, target)
        ssim_loss_sum += ssim_loss
    rmse_gradient_mean_loss = rmse_gradient_loss_sum / batch_size
    ssim_mean_loss = ssim_loss_sum / batch_size
    # print(f"depth image bad pixel rmse_gradient_mean_loss: {rmse_gradient_mean_loss:.2f}")
    # print(f"depth image bad pixel ssim_mean_loss: {ssim_mean_loss:.2f}")
    # total_loss = log(mse_mean_loss) + ssim_mean_loss
    # total_loss = rmse_gradient_mean_loss + ssim_mean_loss
    # print(f"depth image bad pixel total_loss: {total_loss:.2f}")
    # if rmse_gradient_mean_loss!=0:
        # rmse_gradient_mean_loss = log(rmse_gradient_mean_loss)
    # if ssim_mean_loss!=0:
        # ssim_mean_loss = log(ssim_mean_loss)
    return rmse_gradient_mean_loss, ssim_mean_loss
...
def build_targets(p, targets, model):
    ...
    # print(f"model.yolo_layers - {model.yolo_layers}")
    anchors_list = [torch.Tensor([[ 3.62500,  2.81250],
                                    [ 4.87500,  6.18750],
                                    [11.65625, 10.18750]]),
                    torch.Tensor([[1.87500, 3.81250],
                                    [3.87500, 2.81250],
                                    [3.68750, 7.43750]]),
                    torch.Tensor([[1.25000, 1.62500],
                                    [2.00000, 3.75000],
                                    [4.12500, 2.87500]]),
                                    ]
    for i, j in enumerate([89, 101, 113]):
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, multi_label=True, classes=None, agnostic=False):
    ...
    # print(f"nms - prediction shape - {prediction[0].shape}")
    # print(f"nms nc - {nc}")
...
def midas_post_processing(depth_out):
    bits=2
