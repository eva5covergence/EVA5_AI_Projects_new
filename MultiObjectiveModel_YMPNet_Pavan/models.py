...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p, out):
        ...
        if self.training:
            ...
            io[..., :4] *= self.stride
            torch.sigmoid_(io[..., 4:])
            # print("dbg3 inference")
            # print(f"p.shape - {p.shape}")
            # print(f"io.view(bs, -1, self.no) - {io.view(bs, -1, self.no).shape}")
            return io.view(bs, -1, self.no), p  # view [1, 3, 13, 13, 85] as [1, 507, 85]
            # return p

class MidasNetEncoder(BaseModel):
    """Network for monocular depth estimation.
    """

    def __init__(self, cfg, path=None, img_size=(416, 416), features=256, non_negative=True):
        """Init.
        Args:
        ...
        super(MidasNetEncoder, self).__init__()
